//[ESP32 아두이노 코드] 통신 두절 시 자동 정지(Fail-safe) 추가
...
// [안전장치 추가] 마지막으로 명령을 받은 시간을 기록하는 변수
unsigned long lastCommandTime = 0;
const unsigned long TIMEOUT_MS = 500; // 0.5초(500ms) 동안 명령이 없으면 통신 단절로 간주
...
void loop() {
  ...
  // [안전장치 실행] 마지막 명령을 받은 지 0.5초가 넘었다면? 통신이 끊긴 것으로 간주하고 강제 정지
